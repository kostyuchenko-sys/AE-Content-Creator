...
2. **Базовая панель-расширение (extension)**
   ...
   - **Критерий готовности**: пользователь может, не заходя в меню скриптов, нажать кнопку в панели и получить готовую композицию из `TEMPLATE_MAIN` по выделенным футажам.
...
3. **Система шаблонов и конфигов**
   ...
   - **Критерий готовности**: можно добавить новый шаблон, просто создав конфиг и AE-проект (без правок кода; копирование конфигов до CEP-папки делает post-commit hook).
...
- **Current status**: `active` — план утвержден, epic в работе.
- **Next actions**:
  - Уточнить целевую версию(и) After Effects (важно для выбора UXP vs CEP).
  - Начать с PoC (шаг 1 плана): минимальный скрипт замены плейсхолдеров.
  - По мере прогресса отмечать выполненные шаги в чек-листе и добавлять события в `Log`.
